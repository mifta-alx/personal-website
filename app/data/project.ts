import type { Project } from "~/types/project";

const projects: Project[] = [
    {
        slug: "siam",
        name: "SIAM",
        title: "SIAM : Sistem Informasi Akademik Mahasiswa",
        type: "Web Platform",
        idea: "Redeveloping the academic ecosystem to align with modern administrative workflows",
        description: "Collaborating with the PUSTIK team, I worked on a complete re-engineering of the student academic portal to bridge the gap between the outdated legacy system and the current campus administrative requirements. This project involved rebuilding the platform from scratch to fix existing bugs and implement new, synchronized workflows for course registration and academic tracking using a more robust and modern tech stack.",
        cover: "siam-1.webp",
        framework: "Next",
        role: "Frontend Developer",
        technologies: ["Javascript", "React", "Next", "Tanstack", "Tailwind", "Redis"],
        gallery: [
            "siam-1.webp",
            "siam-2.webp",
            "siam-3.webp",
            "siam-4.webp",
            "siam-5.webp",
            "siam-6.webp",
            "siam-7.webp",
            "siam-8.webp",
            "siam-9.webp",
        ]
    },
    // {
    //     slug: "my-itn",
    //     name: "Siakad Master",
    //     title: "My ITN : Sigle Sign On Portal",
    //     type: "Web Platform",
    //     idea: "Centralizing campus digital ecosystem into a seamless Single Sign-On portal",
    //     description: "Working with the PUSTIK team, I focused on overhauling the campus's authentication process to move away from isolated systems into a single integrated portal. I was responsible for building an intuitive interface from the ground up, ensuring a smooth transition for users and making navigation between complex internal applications feel effortless.",
    //     cover: "my-itn-1.webp",
    //     framework: "Next",
    //     role: "Frontend Developer",
    //     technologies: ["Javascript", "React", "Next", "Tanstack", "Tailwind", "Redis"],
    //     gallery: [
    //         "my-itn-1.webp",
    //         "my-itn-2.webp",
    //         "my-itn-3.webp",
    //         "my-itn-4.webp",
    //         "my-itn-5.webp",
    //     ]
    // },
    {
        slug: "my-itn",
        name: "My-ITN",
        title: "My ITN : Sigle Sign On Portal",
        type: "Web Platform",
        idea: "Centralizing campus digital ecosystem into a seamless Single Sign-On portal",
        description: "Working with the PUSTIK team, I focused on overhauling the campus's authentication process to move away from isolated systems into a single integrated portal. I was responsible for building an intuitive interface from the ground up, ensuring a smooth transition for users and making navigation between complex internal applications feel effortless.",
        cover: "my-itn-1.webp",
        framework: "Next",
        role: "Frontend Developer",
        technologies: ["Javascript", "React", "Next", "Tanstack", "Tailwind", "Redis"],
        gallery: [
            "my-itn-1.webp",
            "my-itn-2.webp",
            "my-itn-3.webp",
            "my-itn-4.webp",
            "my-itn-5.webp",
        ]
    },
    {
        slug: "erp",
        name: "ERP Frozen Food",
        title: "ERP Frozen Food",
        type: "Web Platform",
        idea: "Simulating enterprise-grade workflows for integrated business management",
        description: "Developed as an academic research project to deeply understand ERP structures, I built a comprehensive management platform for a frozen food business. I focused on translating complex Odoo inspired workflows such as material procurement, manufacturing cycles, and sales tracking into a functional and intuitive web interface.",
        cover: "erp-1.webp",
        framework: "Remix",
        role: "Frontend Developer",
        technologies: ["Javascript", "Remix", "Tailwind"],
        gallery: [
            "erp-1.webp",
            "erp-3.webp",
            "erp-4.webp",
            "erp-6.webp",
            "erp-7.webp",
            "erp-8.webp",
            "erp-9.webp",
            "erp-11.webp",
            "erp-12.webp",
            "erp-14.webp",
            "erp-15.webp",
            "erp-16.webp",
        ]
    },
    {
        slug: "kickmeup",
        name: "KickMeUp",
        title: "KickMeUp",
        type: "Mobile App",
        idea: "Crafting an immersive and modern mobile shopping experience for sneaker enthusiasts",
        description: "Built to explore the nuances of mobile commerce, KickMeUp prioritizes a smooth, app-like feel for browsing premium footwear. The focus was on creating a highly visual product discovery journey, allowing users to navigate through collections and product details with fluid transitions and a clean, modern aesthetic.",
        cover: "kickmeup-1.webp",
        framework: "React-Native",
        role: "Frontend Developer",
        technologies: ["Javascript", "React-Native"],
        gallery: [
            "kickmeup-1.webp",
            "kickmeup-2.webp",
            "kickmeup-3.webp",
            "kickmeup-4.webp",
        ]
    },
    {
        slug: "pkpu",
        name: "PKPU",
        title: "PKPU : Penundaan Kewajiban Pembayaran Utang",
        type: "Web Platform",
        idea: "Transitioning legal debt restructuring from manual paperwork to a secure digital system",
        description: "Legal administrative tasks often struggle with manual errors and slow processing. This project tackled those issues by digitizing the PKPU (Debt Payment Obligation Suspension) workflow. The result is a centralized system that tracks legal timelines and debt data more accurately, making the entire restructuring process faster and more transparent.",
        cover: "pkpu-1.webp",
        framework: "Sveltekit",
        role: "Fullstack Developer",
        technologies: ["Javascript", "Sveltekit", "Tailwind"],
        gallery: [
            "pkpu-1.webp",
            "pkpu-2.webp",
            "pkpu-3.webp",
            "pkpu-4.webp",
        ]
    },
    {
        slug: "parama-bara",
        name: "Parama Bara Intercontinental",
        title: "Parama Bara Intercontinental Company Profile",
        type: "Web Platform",
        idea: "Establishing a professional digital identity for a global energy commodities trader",
        description: "For an international coal trading firm like PT Parama Bara Intercontinental, digital credibility is key. I built a corporate profile that acts as a global gateway, presenting their operational scale and core values through a sophisticated interface designed to build trust with partners and clients across various countries.",
        cover: "parama-bara-1.webp",
        framework: "Sveltekit",
        role: "Frontend Developer",
        technologies: ["Javascript", "Sveltekit", "Tailwind"],
        gallery: [
            "parama-bara-1.webp",
            "parama-bara-2.webp",
            "parama-bara-3.webp",
            "parama-bara-4.webp",
        ]
    },
    {
        slug: "major-app",
        name: "MajorApp",
        title: "MajorApp: Decision Support System",
        type: "Web Platform",
        idea: "Implementing decision-making algorithms to help students choose their high school majors",
        description: "Choosing a high school major is often a confusing process for students. MajorApp solves this by applying the Simple Additive Weighting (SAW) method to analyze student interests and academic performance. The system processes these variables to generate objective, ranked recommendations, helping students make more informed decisions about their future.",
        cover: "major-1.webp",
        framework: "Laravel",
        role: "Fullstack Developer",
        technologies: ["Php", "Laravel", "Livewire", "Tailwind"],
        gallery: [
            "major-1.webp",
            "major-2.webp",
            "major-3.webp",
            "major-4.webp",
            "major-5.webp",
            "major-6.webp",
            "major-7.webp",
            "major-8.webp",
            "major-9.webp",
        ]
    },
];

const frameworkType = {
    "Typescript": "devicon:typescript",
    "Javascript": "devicon:javascript",
    "Next": "devicon:nextjs",
    "Nuxt": "devicon:nuxt",
    "Sveltekit": "devicon:svelte",
    "React-Native": "devicon:reactnative",
    "React": "devicon:react",
    "Remix": "devicon:remix",
    "Tailwind": "devicon:tailwindcss",
    "Redis": "devicon:redis",
    "Laravel": "devicon:laravel",
    "Livewire": "devicon:livewire",
    "Php": "devicon:php",
    "Tanstack": "logos:react-query-icon"
}

const iconSize = {
    "Typescript": "18",
    "Javascript": "18",
    "Next": "18",
    "Nuxt": "18",
    "Sveltekit": "14",
    "React-Native": "14",
    "React": "14",
    "Remix": "12",
    "Tailwind": "18",
    "Redis": "18",
    "Laravel": "14",
    "Livewire": "14",
    "Php": "14",
    "Tanstack": "14",
}

export { projects, frameworkType, iconSize };